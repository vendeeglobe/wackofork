<?php

$wakkaResource = array(
  // logged in panel
  "YouAre"      => "Voçê é ",
  "YouArePanelLink" => "Parâmetros",
  "YouArePanelName" => "Parâmetros",
  "LogoutLink"  => "Desligar",
  "LogoutButton"  => "Desligar",
  "LogoutAreYouSure"  => "Quer mesmo desligar?",
  // registration panel
  "RegistrationWelcome" => "Registo de Novo Utilizador:",
  "RegistrationName" => "Dê a si mesmo um ((WackoWiki:WackoDocumentation/WhatIsNameSurname NameSurname))",
  "RegistrationPassword" => "Password (5+ caracteres)",
  "RegistrationButton" => "Registe-me!",
  "RegistrationNameOwned" => "Este Nome de Utilizador já está registado. Por favor escolha outro.",
  // login panel
  "LoginWelcome" => "Login",
  "LoginWelcome2" => " . . . . . . . . [[/Registration Register]]",
  "LoginName" => "((WackoWiki:WackoDocumentation/WhatIsNameSurname NameSurname))",
  "LoginPassword" => "Password",
  "LoginButton" => "Login!",
  // other
  "TopicSearchText" => "Procurar apenas nos títulos",
  "MsWordVersion" => "versão preparada para Microsoft Word",
  "MailHello" => "Olá, ",
  "MailGoodbye" => "Atenciosamente, ",
  "A watched Page changed!" => "Página Alterada ",
  "Someone changed this page:" => " alterou esta página que está a acompanhar: ",
  "ActionDenied" => "Acção negada aqui.",
  "TOCTitle" => "Conteúdos ",
  "SearchButtonText" => "Pesquisa",
  "OuterLink2" => "Outgoing link\n(in new window)",
  "MailLink" => "Escrever um Email\n(mail client launch)",
  "ShowTip" => "Clique para voltar ao modo ver",
  "ShowText" => "Ver",
  "ShowDateTimeInLinks" => "Adicione data e hora aos links",
  "Typografica" => "Correcção Ortográfica enquanto vê",
  "Comments_0"    => "Página sem Comentários.",
  "Comments_1"    => "Página com UM comentário.",
  "Comments_n"    => "Há %1 comentários nesta página.",
  "Comments_all"    => "Comentários",
  "ShowComments"    => "Mostrar Comentários e Formulário",
  "HideComments"    => "Fechar Comentários e Formulário",
  "AttachComment"   => "Anexar comentário a esta página:",
  "AttachCommentButton" => "Anexar comentário",

  "DoesNotExists"   => "Esta página ainda não existe. Talvez a queira <a href=\"%1\">criar</a> agora?",
  "Revision"    => "Esta é uma revisão antiga de <a href=\"%1\">%2</a> de %3.",
  "ReadAccessDenied"  => "<em>Não está autorizado a ver esta página.</em>",
  "YouAreOwner"   => "Voçê é o dono desta página.",
  "Owner"     => "Dono: ",
  "Nobody"    => "Ninguém",
  "TakeOwnership"   => "Tomar Posse",
  "DeleteConfirm"   => "Quer mesmo APAGAR a página?\\nNão poderá voltar atrás!.",
  "DeleteTip"   => "Clique aqui para apagar.",
  "DeleteText"    => "(apagar)",
  "EditACLConfirm"  => "Quer mesmo sair sem guardar?",
  "EditACLText"   => "Editar ACLs (permissões)",
  "EditTip"   => "Clique para editar esta página." ,
  "EditText"    => "Edite esta página" ,
  "RevisionTip"   => "Clique para ver revisões recentes",
  "RevisionXMLTip"  => "Clique para ver revisões recentes em formato XML.",
  "ReferrersTip"    => "Clique para ver uma lista de URLs que referenciam esta página",
  "ReferrersText"   => "Referenciadores",
  "SearchText"    => "Pesquisa: ",

  "ACLUpdated"    => "Permissões (ACL) actualizadas",
  "ACLGaveOwnership"  => " e deu posse a ",
  "ACLRead"   => "<strong>Permissão de Leitura (Read ACL):</strong>",
  "ACLWrite"    => "<strong>Permissão de Escrita (Write ACL):</strong>",
  "ACLComment"    => "<strong>Permissão de Comentar (Comments ACL):</strong>",
  "ACLFor"    => "Permissões (ACL) para %1",
  "SetOwner"    => "<strong>Definir Dono:</strong>",
  "OwnerDon'tChange"  => "Não altere",
  "ACLStoreButton"  => "Guardar Permissões (ACLs)",
  "ACLCancelButton" => "Cancelar",
  "ACLAccessDenied" => "<em>Voçê não é o dono desta página.</em>",

  "EditStoreButton" => "Guardar",
  "EditRe-EditButton" => "Re-Editar",
  "EditCancelButton"  => "Cancelar",
  "EditPreviewButton" => "Pré-visão",
  "EditPreview"   => "Pré-visão",
  "OverwriteAlert"  => "ALERTA DE SOBREPOSIÇÃO: Esta página foi modificada por outra pessoa enquanto voçê estava a editá-la.<br />Por favor copie as suas alterações e re-edite esta página.",
  "WriteAccessDenied"   => "<em>Voçe não tem permissão de escrita nesta página.</em>",
  "EmptyComment"    => "O corpo do comentário estava vazio -- não guardado!",
  "CommentAccessDenied"   => "<em>Desculpe, não tem permissão para enviar comentários nesta página.</em>",
  "YouAreNowTheOwner" => "Voçê é agora o dono desta página.",
  "SimpleDiff"    => "Diferenças simples",
  "ShowDifferencesButton" => "Mostrar Diferenças",
  "CancelDifferencesButton"=> "Voltar ao Nó (Node) / Cancelar",
  "Comparison"    => "<b>Comparação para %3 de %1 e %2</b>",
  "SimpleDiffAdditions" => "<b>Adições:</b>",
  "SimpleDiffDeletions" => "<b>Eliminações:</b>",
  "History"   => "História",
  "ReEditOldRevision" => "Re-editar esta revisão antiga",
  "ExternalPages"   => "Páginas externas com ligação a  %1 (%2) (<a href=\"%3\" >ver lista de domínios</a>):",
  "ExternalPages(Global)" => "Sites com links para nós (<a href=\"%1\">veja lista de domínios</a>):",
  "Last24Hours"   => "Últimas 24 horas",
  "LastDays"    => "Últimos %1 dias",
  "Domains/SitesPages"  => "Domínios/páginas com ligações a   %1 (%2)  (<a href=\"%3\" >ver lista de URLs diferentes</a>):",
  "Domains/SitesPages(Global)" =>"Domínios/páginas com ligações a nós(<a href=\"%1\">ver lista de URLs diferentes</a>):",
  "ViewReferringSites"  =>"<a href=\"%1\">Ver sites que referenciam  %2 apenas</a> ",
  "ViewReferrersFor"  =>" <a href=\"%1\">Ver referenciadores a  %2 apenas</a>",
  "ViewReferringSites(Global)"=>"<a href=\"%1\">Ver sites referenciadores globais</a>",
  "ViewReferrersFor(Global)"=>"<a href=\"%1\">Ver referenciadores globais</a>",
  "NoneReferrers"   =>"<em>Nenhum</em>",
  "NotOwnerAndCanDelete"  =>"<i>Se não é o dono desta página não pode apagá-la</i>",
  "ReferrersRemoved"  =>"<i>Referenciadores para a página %1 removidos.</i>",
  "LinksRemoved"    =>"<i>Ligações da página %1 removidas.</i>",
  "AclsRemoved"   =>"<i>Permissões da página %1 removidas.</i>",
  "PageRemoved"   =>"<i>Página %1 removida.</i>",
  "ThisActionHavenotUndo" =>"<h3>!Esta acção não pode ser anulada. Boa Sorte!</h3>",
  "PrintVersion" => "Versão Impressora",
  "ReferringPages" => "Páginas Referenciadoras",
  "NoReferringPages" => "<em>Não existem Páginas Referenciadoras</em>",
  "NoAccessToSourcePage" => "<em>Não está autorizado a ler código da página.</em>",
  "SourcePageDoesntExist" => "Página de código ainda não existe",
  "NotLoggedInThusEdited" => "<em>Voçê não efectuou login, por isso não podemos identificar as páginas que editou.</em>",
  "DidntEditAnyPage" => "<em>Ainda não editou nenhuma página.</em>",
  "NoPagesFound" => "<em>Nenhuma página encontrada.</em>",
  "MyChangesTitle1" => "Esta é a lista de páginas que já editou, ordenadas pela data da alteração mais recente",
  "MyChangesTitle2" => "Esta é a lista de páginas que já editou, junto com a data da sua última alteração",
  "OrderABC" => "Ordem alfabética",
  "OrderDate" => "Ordem por data",
  "OrderChange" => "Ordem ppor alteração",
  "ListOwnedPages" => "Esta é a lista de páginas das quais é dono (por ordem alfabética)",
  "ListOwnedPages2" => "Esta é a lista de páginas das quais é dono (por datas)",
  "ListOwnedPages3" => "Esta é a lista de páginas das quais é dono (por ordem de alterações)",
  "YouDontOwn" => "<em>Não é dono de nenhuma página.</em>",
  "NoPagesFound" => "<em>Nenhuma página encontrada.</em>",
  "NotLoggedInThusOwned" => "<em>Não efectuou o login, por isso não podemos recolher as suas páginas!.</em>",
  "NoOrphaned" => "<em>Nenhuma página órfã.</em>",
  "NoRecentComments" => "<em>Nenhum comentário recente.</em>",
  "LatestCommentBy" => "último comentário por",
  "NoRecentlyCommented" => "<em>Nenhuma página comentada recentemente.</em>",
  "SearchResults" => "Resultados da pesquisa de ",
  "NotFound" => "não encontrado",
  "NoResultsFor" => "Nenhum resultado para ",
  "SearchResultsFor" => "Resultados da pesquisa de ",
  "SearchFor" => "Pesquisa de ",
  "TopicSearchResults" => "Tópicos dos resultados da pesquisa de ",
  "NotFoundInTopics" => "nada encontrado nos tópicos",
  "PagesLinkingTo" => "Páginas ligando a ",
  "NoPageLinkingTo" => "Nenhuma página ligando a ",
  "NoWantedPages" => "<em>Nenhuma página pretendida</em>",
  "RemoveWatch" => "Remover acompanhamento desta página",
  "SetWatch" => "Acompanhar esta página",
  "history" => "história",
  "SettingsStored" => "Parâmetros do Utilizador guardados!",
  "LoggedOut" => "Está desligado.",
  "Hello" => "Olá",
  "YourEmail" => "O seu endereço email",
  "YourMotto" => "O seu mote",
  "RevisionListLimit" => "Limite da lista de revisão",
  "RecentChangesLimit" => "Limite das Alterações recentes",
  "ShowComments?" => "Por defeito mostra comentários",
  "DoubleclickEditing" => "Edição por duplo clique",
  "UpdateSettingsButton" => "Actualizar Parâmetros",
  "WrongPassword" => "Password errada!",
  "AlreadyRegistered" => "Se já é um utilizador registado, faça aqui o seu login!",
  "YourLogin" => "O seu username",
  "Password" => "Password (5+ caracteres)",
  "StuffYouNeed" => "Coisas que só precisa de inserir na primeira vez que faz login (e assim registando-se como novo utilizador neste site).",
  "ConfirmPassword" => "Confirme password",
  "Email" => "Email",
  "MustBeWikiName" => "O Username deve ser do formato((http://wackowiki.com/wackodocumentation/WikiName WikiName)) !",
  "SpecifyEmail" => "Deve inserir endereço email.",
  "NotAEmail" => "Esse não nos parece um endereço válido.",
  "PasswordsDidntMatch" => "As Passwords não coincidiram.",
  "SpacesArentAllowed" => "Não são permitidos espaços nas passwords.",
  "PasswordTooShort" => "Password com poucos caracteres.",
  "SeeListOfPages" => "Veja uma lista das páginas que possui(MyPages) e das páginas que editou (MyChanges).",
  "AddToBookmarks" => "Adicionar página actual aos QuickLinks",
  "YourBookmarks" => "Os seus QuickLinks",
  "YourLanguage" => "Seleccione Idioma",
  "ShowSpaces" => "Mostrar espaço em WikiNames",
  "WrongPage4Redirect" => "Impossível redirecionar por causa de parâmetro errado",
  "ReallyDelete" => "Quer mesmo apagar a página?<br />Não poderá deppois anular esta acção!<br /><br />AVISO: ao Apagar esta página irá eliminar todas as suas revisões, comentários, permissões e referenciadores.",
  "RemoveButton" => "Sim, Apague!",
  "Comment for watched page" => "Novo comentário para página ",
  "Someone commented" => " comentou esta página que está a acompanhar: ",
  "AlertReferringPages" => "Esta página é referenciada por estas páginas",

  "EditIcon" => "(edit)",
  "LinksTree:Level>4 warning" => "A árvore de Links não suporta mais de 4 niveis",
  "LinksTree:Title" => "Árvore de Links",
  "Tree:ClusterTitle" =>"Árvore do cluster %1",
  "Tree:SiteTitle" =>"Árvore do Site",
  "Tree:Empty" => "%1&nbsp;não tem sub-páginas",
  "search_title_help" => "",

  "RenameText"=>"Renomear...",
  "NewName"=>"Insira um novo nome para esta página",
  "RenameButton"=>"Renomear!",
  "AlredyExists"=>"<em>Página <strong>%1</strong> já existente.</em>",
  "AlredyNamed"=>"<em>Página tem o nome <strong>%1</strong>.</em>",
  "AclsRenamed"   =>"<em>Permissões (acls)da página %1 renomeadas.</em>",
  "PageRenamed"   =>"<em>Página %1 renomeada.</em>",
  "NewNameOfPage" =>"Novo nome desta página: ",
  "NotOwnerAndCantRename"  =>"<em>Não sendo dono desta página não pode renomeá-la</em>",
  "NeedRedirect"=>"Criar redirecionamento para nova página na página antiga",
  "RedirectCreated"=>"<em>Na página %1 criado redirecionamento para a nova página</em>",
  "LinksRenamed"   =>"<em>Links da página %1 renomeada.</em>",
  "ChooseTheme" => "Escolha Visual",

  "CreatePage" => "Criar esta página",
  "RemoveFromBookmarks" => "Remover dos favoritos",
  "DontRedirect" => "Não permitir redireccionamento automático<br /> por acção de redirecionamento.",
  "SendWatchMail" => "Enviar notificação por email quando alguém <br /> (excepto voçê) alterar a página que estiver a acompanhar",
  "PageMoved" => "Esta página mudou de localização. Pode encontrá-la em",
  "CurrentPassword" => "Password actual",
  "NewPassword" => "Nova password<br />(5+ caracteres)",
  "YouWantChangePassword" => "Alterar password",
  "PasswordChanged" => "Password alterada",
  "Mail.Welcome" => "Registou-se em ",
  "Mail.Registered" => "Registou-se com sucesso em %1.<br />O seu username: %2<br />Se quiser receber notificações , é necessário aprovar o seu email. Siga este <a href=%3>link</a> para o fazer.",
  "Mail.Verify" => "Alguém (talvez voçê) submeteu o seu email como alvo de notificação no site  %1.<br />O seu username: %2<br />",
  "Mail.Confirm" => "Por favor verifique o seu email",
  "EmailConfirmed" => "Email confirmado com sucesso. Obrigado!",
  "EmailNotConfirmed" => "Email não confirmado. Talvez código errado?",

  "ForgotComment" => "Insira o seu login ou endereço e-mail,\nDepois ser-lhe-á enviado e-mail com instruções.",
  "ForgotMain" => "Password esquecida?",
  "ForgotField" => "((WackoWiki:WackoDocumentation/WhatIsNameSurname NameSurname)) ou e-mail",
  "SendButton" => "Enviar!",
  "UserNotFound" => "<em>Não foi encontrado utilizador com este nome ou e-mail.</em>",
  "NotConfirmedMail" => "<em>O seu e-mail não foi confirmado.<br />Mail não pode ser enviado para endereços não confirmados.</em>",
  "Mail.ForgotSubject" => "Renovação de Password para ",
  "Mail.ForgotMessage" => "Alguém (talvez voçê) esqueceu-se da password para  %1.<br />O seu login: %2<br />Clique no link ou copie para a linha de endereço do seu browser para mudar a sua password.<br /><a href=\"%3\">%3</a>",
  "CodeWasSent" => "Instruções para recuperação de Password foram-lhe enviadas por e-mail.",
  "WrongCode" => "<em>Wrong code</em>",
  "YouWantChangePasswordForUser" => "%1 viu a sua password alterada",

  "Watchers" => "Lista de quem acompanha %1",
  "NoWatchers" => "<em>Não ninguém a acompanhar %1</em>",
  "NotOwnerAndViewWatchers"  =>"<em>Não sendo o dono desta página não pode ver a lista de que a acompanha.</em>",
  "MeasuredTime" => "Tempo",

  //Settings
  "SettingsFor"    => "Parâmetros de  %1",
  "metaDesc" => "META Keywords e descrições que são automaticamente inseridos nos headers do HTML.",
  "meta1" => "Meta Keywords",
  "meta2" => "Meta Description",
  "SettingsText" => "Parâmetros",
  "SettingsTip"  => "Outros parâmetros e propriedades desta página",
  "SettingsPortal" => "Outros parâmetros e propriedades desta página:",
  "SettingsEdit"      => "Editar",
  "SettingsRevisions" => "Revisões",
  "SettingsRename"    => "Renomear",
  "SettingsRemove"    => "Remover",
  "SettingsAcls"      => "ACLs",
  "SettingsUpload"    => "Upload",
  "SettingsReferrers" => "Referrers",
  "SettingsWatch"     => "Acompanhe",
  "SettingsPrint"     => "Versão Impressora",
  "SettingsMsword"    => "versão MS Word",
  "SettingsMassAcls"  => "ACLs para todo cluster",
  "BewareChangeLang"  => "Não altere de Idioma, se não sabe exactamente o que está a fazer. Alterar o Idioma provocará o renomear da página de acordo com as regras de ortografia de cada idioma. Muito conteúdo poderá tornar-se ilegível. Se isso acontecer reverta a situação voltando a escolher o idioma anterior.",
  "SetLang" => "Idioma para esta página",

  // upload
  "UploadFor"  => "Upload do Ficheiro",
  "UploadButtonText" => "Prossiga",
  "UploadGlobalText" => "para acesso público",
  "UploadHereText"   => "apenas para utilizadores autorizados a ver a página actual",
  "UploadDesc" => "descrição",
  "UploadForbidden"  => "Não está autorizado a fazer upload de ficheiros aqui.",
  "UploadNoFile"     => "Não existe nenhum ficheiro no formulário.",
  "UploadMaxSizeReached" => "Ficheiro com tamanho excessivo!",
  "UploadNotAPicture"    => "Só há permissões para upload de imagens.",
  "UploadMaxFileCount"   => "Atinngiu o seu limite de upload.",
  "UploadDone"    => "Upload do Ficheiro com sucesso",
  "UploadKB"   => "Kb",
  "UploadFileNotFound"   => "File não encontrado",
  "UploadFileForbidden"   => "Acesso ao ficheiro proibido",
  "UploadTitle"   => "Ficheiros acessíveis a partir desta página",
  "UploadTitleGlobal"   => "Ficheiros acessíveis a partir de qualquer lado",
  "UploadRemove"   => "remover",
  "UploadRemovedFromDB"    => "Ficheiro removido da base de dados",
  "UploadRemovedFromFS"    => "Ficheiro apagado do servidor fisico de alojamento",
  "UploadRemovedFromFSError"    => "Parece ser impossível apagar este ficheiro!",
  "UploadRemoveConfirm"    => "Quer mesmo apagar este ficheiro?",
  "UploadRemoveDenied"     => "Não está autorizado a remover este ficheiro",
  "UploadMax"     => "Não mais do que ",

  "Files_0"    => "Não existem ficheiros neste página.",
  "Files_1"    => "Há um ficheiro nesta página.",
  "Files_n"    => "Existem %1 ficheiros nesta página.",
  "Files_all"  => "Ficheiros",
  "ShowFiles"  => "Mostrar Ficheiros",
  "HideFiles"  => "Esconder Ficheiros",
  "ShowFiles?" => "Por defeito mostra lista de ficheiros",

  "RegistrationLang" => "O seu idioma",
  "BadName" => "Nome escolhido não é válido",
  "PleaseLogin" => "((Settings Login aqui))",
  "Bookmarks" => "Favoritos",
  "ReallyDeleteComment" => "Quer mesmo apagar comentário?<br />Depois não poderá voltar atrás!",
  "ForgotLink" => "Password esquecida?",
  "RegistrationClosed" => "O registo não está disponível. Tente contactar o administrador.",
  "ACLForCluster"    => "Lista de Permissões para TODO o cluster %1",
  "ACLAreYouSure" => "Vai alterar aas permissões desta página e todas as suas subpáginas. Quer continuar?",

  "MeasuredMemory" => "Memória",

  "ImportTo" => "Import in cluster",
  "ImportWhat" => "XML-file for import",
  "ImportButtonText" => "Import",
  "ImportSuccess" => "Imported succesfully.",
  "MassRenaming" => "Rename all cluster",
  "MassNewName" => "Enter new name for this cluster",
  "MassNeedRedirect"=>"Create redirect to the new pages on the old pages",
  "SettingsMassAcls"  => "for entire cluster",
  "SettingsMassRename"  => "for entire cluster",
  
  "By" => "by",
  "UserGroups" => "Groups of users",

  "Next" => "Next",
  "Prev" => "Prev",

  "default_bookmarks" => "PageIndex\nRecentChanges\nRecentlyCommented\n((Users))\n((Registration))",
  "site_bookmarks" => "ÍndicedePáginas / AlteraçõesRecentes / RecentementeComentadas",

  //icons
  "outericon" => '<img src="{theme}icons/web.gif" alt="" border="0" />',
  "fileicon"  => '<img src="{theme}icons/file.gif" alt="" border="0" />',
  "mailicon"  => '<img src="{theme}icons/mail.gif" alt="" border="0" />',
  "lockicon"  => '<img src="{theme}icons/lock.gif" align="middle" hspace="2" alt="Access denied" border="0" />',
  "keyicon"   => '<img src="{theme}icons/key.gif" align="middle" hspace="2" alt="Page with custom access" border="0" />',
  "wantedicon" => '?',
);


?>