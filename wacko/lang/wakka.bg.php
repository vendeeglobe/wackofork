<?php

$wakkaResource = array(
  // logged in panel
  "YouAre" => "Вие сте ",
  "YouArePanelLink" => "Settings",
  "YouArePanelName" => "Настройки",
  "LogoutLink" => "Изход",
  "LogoutButton" => "Изход от системата",
  "LogoutAreYouSure" => "Излизате ли от системата?",
  // registration panel
  "RegistrationWelcome" => "Регистрация на нов потребител:",
  "RegistrationName" => "Задайте своето ((WackoWiki:DocEnglish/WhatIsNameSurname NameSurname))",
  "RegistrationPassword" => "Парола (не по-къса от 5 символа)",
  "RegistrationButton" => "Карай нататък!",
  "RegistrationNameOwned" => "Желаното име е вече заето!",
  // login panel
  "LoginWelcome" => "Вход",
  "LoginWelcome2" => " . . . . . . . . [[/Регистрация Регистрация]]",
  "LoginName" => "((WackoWiki:DocEnglish/WhatIsNameSurname NameSurname))",
  "LoginPassword" => "Парола",
  "LoginButton" => "Влез!",
  // other
  "TopicSearchText" => "Търси само в заглавните",
  "MsWordVersion" => "Версия за експорт в Microsoft Word",
  "MailHello" => "Здравейте, ",
  "MailGoodbye" => "Винаги Ваш, ",
  "A watched Page changed!" => "Промени в ",
  "Someone changed this page:" => " е променил(а) страницата, която следите: ",
  "ActionDenied" => "Отказан достъп до действието",
  "TOCTitle" => "Заглавие на документа",
  "SearchButtonText" => "Търси",
  "OuterLink2" => "Външна препратка\n(появява се в нов прозорец)",
  "MailLink" => "Напиши писмо\n(извикване на пощенската програма)",
  "ShowTip" => "Кликнете, за да се върнете към прегледа",
  "ShowText" => "Преглед",
  "ShowDateTimeInLinks" => "Добавя дата/време на изпращане",
  "WikieditAutocomplete" => "Complete wikilinks by Ctrl+Space",
  "Typografica" => "Корекция на типографика при прегледа",
  "Comments_0" => "Коментар няма.",
  "Comments_1" => "Един коментар.",
  "Comments_n" => "Коментари (%1).",
  "Comments_all" => "Коментари",
  "ShowComments" => "Покажи коментарите/формата",
  "HideComments" => "Скрий коментарите/формата",
  "AttachComment" => "Добави комментар:",
  "AttachCommentButton" => "Насочи",

  "DoesNotExists" => "Такава страница не съществува. Искате ли <a href=\"%1\">да я съзадете</a> ?",
  "Revision" => "Това е стара версия на <a href=\"%1\">%2</a> от %3.",
  "ReadAccessDenied" => "<em>Вие нямате права за да четете тази страница.</em>",
  "YouAreOwner" => "Собственик: Вие. ",
  "Owner" => "Собственик: ",
  "Nobody" => "Никой",
  "TakeOwnership" => "Стани собственик",
  "DeleteConfirm" => "Действително ли искате да ИЗТРИЕТЕ този текст? \\nПо-късно няма да можете да го възстановите.",
  "DeleteTip" => "Кликнете, за да изтриете.",
  "DeleteText" => "Изтрий",
  "EditACLConfirm" => "Изход без запазване на направените промени?",
  "EditACLText" => "Права за достъп...",
  "EditTip" => "Кликнете, за да промените страницата." ,
  "EditText" => "Редактиране" ,
  "RevisionTip" => "Кликнете, за да видите списъка на промените",
  "RevisionXMLTip" => "Кликнете, за да видите списъка на промените във формат XML.",
  "ReferrersTip" => "Кликнете, за да видите списъка на страниците, препращащи на тази",
  "ReferrersText" => "Препратки",
  "SearchText" => "Търсене: ",

  "ACLUpdated" => "ACL обновен",
  "ACLGaveOwnership" => " и собствеността е променена",
  "ACLRead" => "<strong>Права за четене:</strong>",
  "ACLWrite" => "<strong>Права за запис:</strong>",
  "ACLComment" => "<strong>Права за коментар:</strong>",
  "ACLFor" => "ACL за %1",
  "SetOwner" => "<strong>Установи собственика:</strong>",
  "OwnerDon'tChange" => "Не променяй",
  "ACLStoreButton" => "Запази",
  "ACLCancelButton" => "Отмени",
  "ACLAccessDenied" => "<em>Вие не сте собственик на тази страница.</em>",

  "MetaStoreButton" => "Запази",
  "MetaCancelButton" => "Отмени",

  "EditStoreButton" => "Запази",
  "EditRe-EditButton" => "Продължи редактирането",
  "EditCancelButton" => "Отмени",
  "EditPreviewButton" => "Предварителен преглед",
  "EditPreview" => "Предварителен преглед",
  "OverwriteAlert" => "СЪОБЩЕНИЕ ЗА ПРЕЗАПИС: Тази страница е била променена от някой, докато вие сте я редактирали.<br />Моля, копирайте Вашите промени и редактирайте страницата повторно.",
  "WriteAccessDenied" => "<em>Вие нямате права за промяна на тази страница.</em>",
  "EmptyComment" => "Коментарът е празен -- няма да бъде запазен!",
  "CommentAccessDenied" => "!<em>Извинявайте, но Вие нямате права за коментар на тази страница.</em>",
  "YouAreNowTheOwner" => "Сега вие сте собственик на тази страница."  ,
  "SimpleDiff" => "Просто сравнение",
  "ShowDifferencesButton" => "Сравни",
  "CancelDifferencesButton" => "Върни / Отмени",
  "Comparison" => "<b>Сравнение на версии %3 за %1 и %2</b>",
  "SimpleDiffAdditions" => "<b>Добавено:</b>",
  "SimpleDiffDeletions" => "<b>Извадено:</b>",
  "NoDifferences" => "No differences.",
  "History" => "История на промените",
  "ReEditOldRevision" => "Редактирай старата версия",
  "ExternalPages" => "<strong>Външни страници, препращащи на&nbsp;%1</strong><br /><small> (%2) (<a href=\"%3\" >списък на сайтовете</a>):</small>",
  "ExternalPages(Global)" => "<strong>Външни страници, препращащи на&nbsp;".$wakkaConfig["wakka_name"]."</strong><br /><small> (<a href=\"%1\">списък на сайтовете</a>):</small>",
  "Domains/SitesPages" => "<strong>Сайтове, на които има препратки&nbsp;%1</strong><br /><small> (%2)  (<a href=\"%3\" >списък на отделните страници</a>):</small>",
  "Domains/SitesPages(Global)" => "<strong>Сайтове, на които има препратки&nbsp;".$wakkaConfig["wakka_name"]."</strong><br /><small> (<a href=\"%1\">списък на отделните страници</a>):</small>",
  "Last24Hours" => "за последното денонощие",
  "LastDays" => "за последните %1 дни",
  "ViewReferringSites" => "<a href=\"%1\">Сайтове, препращащи на %2</a>",
  "ViewReferrersFor" => "<a href=\"%1\">Препратки на %2</a>",
  "ViewReferringSites(Global)" => "<a href=\"%1\">Сайтове, препращащи на ".$wakkaConfig["wakka_name"]."</a>",
  "ViewReferrersFor(Global)" => "<a href=\"%1\">Препратки на ".$wakkaConfig["wakka_name"]."</a>",
  "NoneReferrers" => "<em>Нет</em>",
  "NotOwnerAndCanDelete" => "<em>Вие не сте собственик на тази страница и затова не можете да я изтриете</em>",
  "ReferrersRemoved" => "<em>Външните препратки на страница %1 изтрити.</em>",
  "LinksRemoved" => "<em>Локалните препратки от %1 изтрити.</em>",
  "AclsRemoved" => "<em>Правата за достъп %1 изтрити.</em>",
  "PageRemoved" => "<em>Страница %1 изтрита.</em>",
  "ThisActionHavenotUndo" =>"<h3>Изтриването не може да се отмени.</h3>",
  "PrintVersion" => "Версия за печат",
  "ReferringPages" => "Страници, препращащи на дадената",
  "NoReferringPages" => "<em>Няма препратки на тази страница</em>",
  "MostPopularPages" => "Most popular pages",
  "NoAccessToSourcePage" => "<em>Достъп до препращащата страница е забранен от Действията.</em>",
  "SourcePageDoesntExist" => "Страница, извикана от Действията, засега не съществува.",
  "NotLoggedInThusEdited" => "<em>Вие не сте регистрирани в системата, затова промените от Вас страници не могат да бъдат намерени.</em>",
  "DidntEditAnyPage" => "<em>Вие още нищо не сте редактиали.</em>",
  "NoPagesFound" => "<em>Търсенето не доведе до резултати.</em>",
  "MyChangesTitle1" => "Това е списък на променените от Вас страници, сортирани по времето на Вашите промени",
  "MyChangesTitle2" => "Това е списък на променените от Вас страници показвайки времето на Вашите последни промени.", 
  "OrderABC" => "сортиране по азбучен ред",
  "OrderDate" => "сортиране по начална дата",
  "OrderChange" => "сортиране по дата на промяна",
  "ListOwnedPages" => "Страници, ваша собственост (в азбучен ред)",
  "ListOwnedPages2" => "Страници, ваша собственост (по начална дата)",
  "ListOwnedPages3" => "Страници, ваша собственост (по дата на промяна)",
  "YouDontOwn" => "<em>Вие не сте собственик на страница.</em>",
  "NotLoggedInThusOwned" => "<em>Вие не сте регистрирани в системата и затова Вашите страници не могат да бъдат намерени.</em>",
  "NoOrphaned" => "<em>Няма загубени страници. Отлично!</em>",
  "PagesLinkingTo" => "Страници, препращащи на ",
  "NoPageLinkingTo" => "Няма препращащи страници",
  "NoWantedPages" => "<em>Няма незапълнени страници. Отлично!</em>",
  "NoRecentComments" => "<em>Коментари отдавна не е имало.</em>",
  "LatestCommentBy" => "последен коментирал",
  "NoRecentlyCommented" => "<em>Коментари отдавна не е имало.</em>",
  "SearchResults" => "Резултати за търсената фраза ",
  "NotFound" => "не е намерен",
  "NoResultsFor" => "Не са намерени резултати за фразата ",
  "SearchResultsFor" => "Резултати за търсената фраза ",
  "SearchFor" => "Търси",
  "TopicSearchResults" => "Резултати за заглавието на търсената фраза ",
  "NotFoundInTopics" => "няма в заглавието ",
  
  "PostedBy"  => "posted by",
  "ThisIsCommentOn" => "This is a comment on",
  "At" => "at",
  
  "RemoveWatch" => "Спри огледа",
  "SetWatch" => "Оглед",
  "history" => "история",
  "SettingsStored" => "Вашите настройки са запазени",
  "LoggedOut" => "Вие излизате от системата",
  "Hello" => "Здравейте",
  "YourEmail" => "Адрес на електонната поща",
  "YourMotto" => "Вашият девиз",
  "RevisionListLimit" => "Ограничение на списъка на поправките в страницата",
  "RecentChangesLimit" => "Ограничение на списъка на променените страници",
  "ShowComments?" => "Покажи коментариите по подразбиране",
  "DoubleclickEditing" => "Редактирание с двоен клик на мишката",
  "UpdateSettingsButton" => "Запази настройките",
  "WrongPassword" => "Неправилна парола!",
  "AlreadyRegistered" => "Ако Вие сте се регистрирали, влезте тук!",
  "YourLogin" => "Вашето УикиИме",
  "Password" => "Парола (не по-малко от 5 символа)",
  "StuffYouNeed" => "Тези полета трябва да се попълнят, само ако Вие се  регистрирате тук за първи път (така ще бъде създаден нов потребител).",
  "ConfirmPassword" => "Подтвърждение на паролата",
  "Email" => "Адрес на електронната поща",
  "MustBeWikiName" => "Името на потребителя трябва да бъде ((WackoWiki:WackoДокументация/УикиИме УикиИмето)) ",
  "SpecifyEmail" => "Вие трябва да посочите адреса на електронната поща.",
  "NotAEmail" => "Това не е адрес на електронна поща.",
  "PasswordsDidntMatch" => "Паролата не е правилна.",
  "SpacesArentAllowed" => "Не може да се използват празни символи в паролата.",
  "PasswordTooShort" => "Паролата е прекалено къса.",
  "SeeListOfPages" => "Списък на Вашите страници и тези, които сте редактирали.",
  "AddToBookmarks" => "Добави в личното меню",
  "YourBookmarks" => "Вашето лично меню",
  "YourLanguage" => "Изберете език",
  "ShowSpaces" => "Покажи празните символи в УикиИмента",
  "WrongPage4Redirect" => "Преадресирането не е възможно поради неверни параметри на преадресацията",
  "ReallyDelete" => "Вие действително ли искате да ИЗТРИЕТЕ страницата? <br />след това няма да можете да я Възстановите.<br /><br />ВНИМАНИЕ: при изтриването на страницата ще бъдат изтрити всички нейни версии, коментари, референции и т.п.",
  "RemoveButton" => "Да, изтрий!",
  "Comment for watched page" => "Коментар за страницата ",
  "Someone commented" => " коментирал гледаната от вас страница: ",
  "AlertReferringPages" => "Препратка на дадената страница е на страница",
  "UnknownAction" => "Unknown action",

  "EditIcon" => "(редактиране)",
  "LinksTree:Level>4 warning" => "Линковете са макс. 4 нива",
  "LinksTree:Title" => "Дърво на линковете",
  "Tree:ClusterTitle" => "Дърво на СЕКТОР %1",
  "Tree:SiteTitle" => "Дърво на сайта",
  "Tree:Empty" => "%1&nbsp;няма подстраници",
  "search_title_help" => "",
  
  "RenameText" => "Преименувай...",
  "NewName" => "Въведете ново име за тази страница",
  "RenameButton" => "Преименувай!",
  "AlredyExists" => "<em>Страница с име <strong>%1</strong> вече съществува.</em>",
  "AlredyNamed" => "<em>Тази страница вече е с име <strong>%1</strong>.</em>",
  "AclsRenamed" => "<em>Правата за  достъп %1  са преименувани.</em>",
  "PageRenamed" => "<em>Страница %1 е преименувана.</em>",
  "FilesRenamed" => "<em>Page files for %1 renamed.</em>",
  "NewNameOfPage" => "Новото име е: ",
  "NotOwnerAndCantRename" => "<em>Вие не сте собственика на тази страница и не можете да я преименувате</em>",
  "NeedRedirect" => "Създай редирект до тази от старата страница",
  "RedirectCreated" => "<em>На страница %1 е създаден редирект към новата</em>",
  "LinksRenamed" => "<em>Линковете от %1  са преименувани.</em>",
  "ChooseTheme" => "Изберете тема",

  "CreatePage" => "Създай страницата",
  "RemoveFromBookmarks" => "Изтрий от личното меню",
  "DontRedirect" => "Забрани действие Redirect<br /> автоматично пренасочване",
  "SendWatchMail" => "Email <br /> при промяна на наблюдаваните страници",
  "PageMoved" => "Преместено на адрес",
  "CurrentPassword" => "Въведете текущата парола",
  "NewPassword" => "Нова парола<br />(не по малко от  5 символа)",
  "YouWantChangePassword" => "Промяна на паролата",
  "PasswordChanged" => "Паролата е  успешно променена",
  "Mail.Welcome" => "Регистрация ",
  "Mail.Registered" => "Успешна регистрация  %1.<br />Вашия логин е: %2<br />За получаване на информация, трябва да потвърдите адреса си. Кликнете на <a href=%3>този линк</a>.",
  "Mail.Verify" => "Някой, (вероятно Вие) е дал този адрес в %1.<br />Вашия логин е: %2<br />За получаване на информация, трябва да потвърдите адреса си. Кликнете на <a href=%3>този линк</a>.",
  "Mail.Confirm" => "Моля потвърдете вашия email",
  "EmailConfirmed" => "Вашият email е потвърден успешно.",
  "EmailNotConfirmed" => "Проблеми с потвърждението.",

  "ForgotComment" => "Въведете вашия логин или e-mail,\nи ще ви бъде изпратено писмо с инструкции.",
  "ForgotMain" => "Забравена парола?",
  "ForgotField" => "((WackoWiki:DocEnglish/WhatIsNameSurname NameSurname)) or e-mail",
  "SendButton" => "Изпрати!",
  "UserNotFound" => "<em>Няма такъв потребител.</em>",
  "NotConfirmedMail" => "<em>Email адреса не е подтвърден.<br/>Не може да се изпрати поща.</em>",
  "Mail.ForgotSubject" => "Обновяване на парола за ",
  "Mail.ForgotMessage" => "Някой е забравил Вашата парола за %1.<br />Вашия логин е: %2<br />За получаване на нова  парола, моля адресирайте следния линк: <br /><a href=\"%3\">%3</a>",
  "CodeWasSent" => "Инструкцията за паролата е изпратена на email.",
  "WrongCode" => "<em>Неверен код</em>",
  "YouWantChangePasswordForUser" => "Промяна на парола за %1",
  
   "Watchers" => "Списък на следящите %1",
  "NoWatchers" => "<em>Страница %1 не е следена</em>",
  "NotOwnerAndViewWatchers" => "<em>Не сте собственик на %1 и не може да получите списък на хората, които я следят.</em>",

  //Settings
  "SettingsFor" => "Настройки на %1",
  "metaDesc" => "Ключови думи/Описание  (за HTML МЕТА таговете).",
  "meta1" => "Ключови думи",
  "meta2" => "Описание",
  "SettingsText" => "Настройки",
  "SettingsTip" => "Други настройки",
  "SettingsPortal" => "Още настройки на страницата:",
  "SettingsEdit" => "Редактиране",
  "SettingsRevisions" => "Версии",
  "SettingsRename" => "Преименуване",
  "SettingsRemove" => "Изртиване",
  "SettingsAcls" => "Достъп",
  "SettingsUpload" => "Качване на файлове",
  "SettingsReferrers" => "Референции",
  "SettingsWatch" => "Наблюдение",
  "SettingsPrint" => "Версия за печат",
  "SettingsMsword" => "Версия за MS Word",
  "BewareChangeLang" => "Не променяйте езика. След подобна промяна е възможно да не разберете как да върнете нещата обратно.",
  "SetLang" => "Език на тази страница",

  // upload
  "UploadFor" => "Качване на файл",
  "UploadButtonText" => "Качване",
  "UploadGlobalText" => "за всички",
  "UploadHereText" => "само за имащите достъп до тази страница",
  "UploadDesc" => "описание",
  "UploadForbidden" => "Не можете да качите файл тук.",
  "UploadNoFile" => "Няма файл.",
  "UploadMaxSizeReached" => "Файлът е много голям!",
  "UploadNotAPicture" => "Разрешени са само картинки, а Вие какво опитвате?",
  "UploadMaxFileCount" => "С този файл, квотата Ви се надвишава",
  "UploadDone" => "Файлът е качен успешно",
  "UploadKB" => "Кб",
  "UploadFileNotFound" => "Файлът не е намерен",
  "UploadFileForbidden" => "Няма достъп до файла",
  "UploadTitle" => "Файлове, достъпни от  тази страница",
  "UploadTitleGlobal" => "Файлове, достъпни отвсякъде",
  "UploadRemove" => "премахване",
  "UploadRemovedFromDB" => "Файлът е премехнат от базата данни.",
  "UploadRemovedFromFS" => "Файлът е безвъзвратно изтрит.",
  "UploadRemovedFromFSError" => "Файлът не може да се изтрие!",
  "UploadRemoveConfirm" => "Изтриване на файла?",
  "UploadRemoveDenied" => "Не Ви е разрешено премахване на файла",
  "UploadMax" => "Не повече от ",

  "Files_0" => "Няма файлове.",
  "Files_1" => "Един файл.",
  "Files_n" => "Има (%1) файла.",
  "Files_all" => "Файлове",
  "ShowFiles" => "Покажи файловете/формата",
  "HideFiles" => "Скрий файловете/формата",
  "ShowFiles?" => "Покажи списъка на файловете по подразбиране",

  "RegistrationLang" => "Your language (Език)",
  "BadName" => "Избраното име е неправилно",
  "Bookmarks" => "Предпочитано",
  "PleaseLogin" => "((Settings Login Въведете име и парола))",
  "ReallyDeleteComment" => "Наистина  ИЗТРИВАНЕ на комментара? <br />Възстановяването след това е невъзможно.",
  "ForgotLink" => "Забравена парола?",
  "RegistrationClosed" => "Регистрацията на тази страница е невъзможна. Свържете се с администратора.",
  "ACLForCluster" => "Контрол на достъп  за  целия  СЕКТОР %1",
  "ACLAreYouSure" => "Ще се сменят правата за тази страница и всички подстраници. Продължение?",

  "MeasuredTime" => "Време",
  "MeasuredMemory" => "Използвана памет",
  "PoweredBy" => "Powered by",

  "ImportTo" => "Импортировать под адресом",
  "ImportWhat" => "Файл для импорта",
  "ImportButtonText" => "Импортировать",
  "ImportSuccess" => "Импортировано успешно.",
  "MassRenaming" => "Рекурсивное переименование",
  "MassNewName" => "Введите новое имя для этого кластера",
  "MassNeedRedirect" => "Поставить на старых страницых редирект на новые",
  "SettingsMassAcls" => "Права за целия СЕКТОР",
  "SettingsMassRename" => "для всех страниц кластера",

  "By" => "by",

  "Next" => "Вперед",
  "Prev" => "Назад",

  "default_bookmarks" => "((Каталог))\n((ПоследниПромени))\n((НовиКоментари))\n((Потребители))\n((Регистрация))",
  "site_bookmarks" => "((Каталог)) / ((ПоследниПромени)) / ((НовиКоментари))",
  //icons
  "outericon" => '<img src="{theme}icons/web.gif" alt="" />',
  "fileicon" => '<img src="{theme}icons/file.gif" alt="" />',
  "pdficon"  => '<img src="{theme}icons/pdf.gif" alt="PDF Document" />',
  "mailicon" => '<img src="{theme}icons/mail.gif" alt="" />',
  "lockicon" => '<img src="{theme}icons/lock.gif" alt="Вам запрещён доступ" />',
  "keyicon" => '<img src="{theme}icons/key.gif" alt="Страница с ограниченным доступом" />',
  "wantedicon" => '?',
  
  // My Groups
  "UserGroups" => "Группы пользователей",
  "MyGroups" => "Group Associations",
  "Group" => "group",
  "Groups" => "groups",
  
  // Cache
  "ClearCache" => "Clear cache",
  "CacheCleared" => "Cache cleared.",
);

?>