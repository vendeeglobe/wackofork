<?php
$lang = array(
"SecurityRisk" => "Не забравяйте накрая да промените обратно правата за достъп на <tt>wakka.config.php</tt> на сървъра. Въпрос на \"сигурност\"!",
"note" => "ОБЪРНЕТЕ ВНИМАНИЕ: Програмата за инсталиране ще опита да запише настройките във файл <tt>wakka.config.php</tt>, разположен в главната директория, където е WackoWiki. За да стане това, този файл трябва да е достъпен за запис чрез системата сървър/скрипт! За да няма проблеми, трябва сега временно да се променят правата за достъп до (мястото на) този файл.<br /><br />Вж. <a href=\"http://wackowiki.com/WackoDocumentation/Installation\" target=\"_blank\">WackoWiki:WackoDocumentation/Installation</a>.",
"mysqlHostDesc" => "Име (hostname), на MySQL сървъра. Обикновено \"localhost\".",
"mysqlHost" => "Име (hostname) на MySQL",
"dbDesc" => "База данни на MySQL,  за WackoWiki. Трябва да е създадена предварително!",
"db" => "БД MySQL",
"mysqlPasswDesc" => "Име и парола за БД MySQL.",
"mysqlUser" => "Име (username) MySQL",
"mysqlPassw" => "Паролa (password) MySQL",
"metaDesc" => "META тагове. Някои казват, че търсачките ги ползували.",
"meta1" => "Ключови думи",
"meta2" => "Описание",
"ForDetailsSee" => "Отново, погледнете за подробности, малко е на английски, но все пак <a href=\"http://wackowiki.com/WackoDocumentation/Installation\" target=\"_blank\">WackoWiki:WackoDocumentation/Installation</a>",
"review" => "Моля, проверете настройките.",
"pleaseConfigure" => "Моля, настройте вашия WackoWiki .",
"fresh" => "Изглежда като за нова инсталация, няма следи от стар WackoWiki . Ще се инсталира нов WackoWiki ",
"langDesc" => "Изберете език. Той ще се използува за инсталацията, а също и ще стане език по подразбиране на новия WackoWiki.",
"installed" => "Инсталирана WackoWiki версия ",
"toUpgrade" => "Подканваме ви <strong>да подновите</strong> вашия WackoWiki с ",
"Moving data to revisions table..." => "Преместваме старите версии в таблица revisions...",
"langConf" => "Езикови настройки",
"lang" => "Изберете език",
"VeryBad" => "Големи проблеми. Възможна е загуба на данни. Потърсете техническа помощ.",
"DBError" => "Не е открита такава БД. Проверете дали е създадена предварително!",
"NextStep" => "На следващата стъпка, инсталаторът ще опита да запише обновения конфигурационния файл,",
"MakeWrite" => "Кофигурационният файл <tt>wakka.config.php</tt> се  разполага в главната директория, където е WackoWiki. За да стане промяната, този файл трябва да е достъпен за запис чрез системата сървър/скрипт! За да няма проблеми, трябва сега временно да се променят правата за достъп до този файл. В противен случай ще трябва да допишете промените на ръка.",
"TestSql" => "Проверка на връзката с БД...",
"Installing Stuff" => "Инсталиране",
"failed" => "ГРЕШКА",
"Testing Configuration" => "Тест на настройките",
"Looking for database..." => "Проверка на БД...",
"pages alter" => "Малка промяна в структурата на таблица pages...",
"0.1.1" => "Изпращаме писмо на създателите...",
"useralter" => "Малка промяна в структурата на таблица  user...",
"Continue" => "Продължение",
"Creating table..." => "Създаваме таблица %1...",
"Already exists?" => "Вече я има?",
"to" => "->",
"Adding some pages..." => "Добавяме първоначално...",
"Hmm!" => "Хм!",
"Claiming all your base..." => "Изпращаме писмо до Бил Гейтс...",
"And table..." => "И таблица %1 (почакайте малко!)...",
"writtenAt" => "записан ",
"dontchange" => "не променяйте wakka_version ръчно!",
"writing" => "<strong>Запазваме настройките</strong><br />\n",
"writing2" => "Запис на конфигурационния файл",
"ready" => "<p>Ура! Изглежда успешно. Сега можете",
"return" => "да се върнете на вашия Уики",
"warning" => "<span class=\"failed\">ВНИМАНИЕ:</span> Конфигурационния файл",
"GivePrivileges" => "не може да се запише. Трябва сега временно да се променят правата за достъп до (мястото на) този файл или да се направи празен файл <tt>wakka.config.php</tt> (<tt>touch wakka.config.php ; chmod 666 wakka.config.php</tt>; не забравяйте накрая да забраните обратно правата, т.e. <tt>chmod 644 wakka.config.php</tt>). Ако нещо не става, ще трябва да копирате и запишете следващия текст в нов празен файл на вашия компютър и после да го прехвърлите с име <tt>wakka.config.php</tt> на сървъра в главната директория на Wacko. При проблеми, вижте (англ.) <a href=\"http://wackowiki.com/WackoDocumentation/Installation\" target=\"_blank\">WackoWiki:WackoDocumentation/Installation</a> или (руски) <a href=\"http://wackowiki.com/WackoDokumetacija/Ustanovka\">WackoWiki:WackoДокументация/Установка</a>",
"try again" => "Нов опит",
"title" => "Инсталация  на WackoWiki",
"databaseConf" => "Настройка на БД",
"prefixDesc" => "Префикс на таблиците на  Wacko. Това дава възможност за няколко WackoWiki на една БД, само чрез различни префикси.",
"prefix" => "Префикс на таблиците",
"SiteConf" => "Настройки  на сайта  WackoWiki",
"nameDesc" => "Име на вашия WackoWiki. Трябва да е от типа УикиИме и изглежда ПримерноЕтоТака.",
"name" => "Посочете името на УакоУики",
"homeDesc" => "Главна страница на WackoWiki.  Трябва да е  с УикиИме.",
"home" => "Главна страница",
"UrlConf" => "Настройка на адресацията",
"baseDesc" => "Базов URL  на вашата инсталация на  WackoWiki. Имената на страниците се присъединяват към него с параметър \"?wakka=\", ако не може да се ползва mod_rewrite.",
"base" => "Базов URL",
"rewriteDesc" => "Rewrite-режим трябва да е включен, ако ползвате mod_rewrite.",
"rewrite" => "Rewrite-режим",
"enabled" => "Включен",
"AdminConf" => "Настройка на администраторския достъп",
"adminDesc" => "Изберете име. Нещо от тип  AdminName (без кирилица).",
"admin" => "Име",
"passwDesc" => "Изберете парола ( 5 и повече символа )",
"password" => "Парола",
"password2" => "Подтвърждение на паролата",
"mailDesc" => "Адрес на електронната поща (email) на администратора.",
"mail" => "Email",
"adding pages" => "Добавяме нови страници...",
"incorrect wikiname" => "Изберете валидно WackoWiki име!",
"incorrect email" => "Въведете истински адрес!",
"passwords don't match" => "Паролата на администратора не съвпадна нещо.",
"password too short" => "много е кратка.",
"adding admin" => "Добавяме административен потребител...",
"Doubles" => "Пробвайте <a href=\"http://wackowiki.com/WackoDokumentacija/CleanupScript\" target=\"_blank\">WackoWiki:WackoДокументация/CleanupScript</a>, това ще ускори достъпа до вашия сайт.",
"newinstall" => "Инсталиращата програма е настроила параметрите. Променяйте ги, ако знаете какво правите!",
"multilangDesc" => "&laquo;Многоезичен&raquo; режим, позволяващ работа на няколко езика. Ако е включен, инсталаторът ще инициализира страници на няколко различни езика.",
"multilang" => "&laquo;Многоезичен&raquo; режим",
"PleaseBackup" => "Моля, съхранете си копие на базата данни (БД) от MySQL, конфигурационния файл и другите, променени от вас файлове (например,  теми),  докато още НЕ Е КЪСНО!",
"apply rights" => "Попытка установить права доступа на папку ",
"apply rights yourself" => "Вы должны самостоятельно установить доступ на запись на папку ",
"RewriteModeAlert" => "ВНИМАНИЕ!\nНастройки базового URL и Rewrite-режима выглядят подозрительно. Обычно при включенном Rewrite не бывает знака ? в базовом URL, но у вас он есть.\n\nДля продолжения инсталляции с текущими настройками нажмите ОК.\nДля того, чтобы вернуться в форму и всё исправить, нажмите ОТМЕНА.\n\nЕсли вы решили идти дальше, помните, что такое сочетание часто является причиной неработоспособности после инсталляции.",
);
?>
