...
<?php
  ...
  if ($_GET["remove"]) // show the form
  {
    ...
    $what = $this->LoadAll( "select user, id, filename, filesize, description from ".$this->config["table_prefix"]."upload where ".
                            "page_id = '".quote($page_id)."' and filename='".quote($_GET["file"])."'");
    if (sizeof($what) > 0)
    {
...
<input name="submit" class="OkBtn_Top" onmouseover='this.className="OkBtn_Top_";' 
       ...
       type="submit" align="top" value="<?php echo $this->GetResourceValue("RemoveButton"); ?>" />
...
<input class="CancelBtn_Top" onmouseover='this.className="CancelBtn_Top_";' 
  ...
  if ($_POST["remove"]) // delete
  {
    ...
    $what = $this->LoadAll( "select user, id, filename, filesize, description from ".$this->config["table_prefix"]."upload where ".
                            "page_id = '".quote($page_id)."' and filename='".quote($_POST["file"])."'");
    if (sizeof($what) > 0)
    {
      if ($this->IsAdmin() || ($page_id && ($this->GetPageOwner($this->tag) == $this->GetUserName()))
                           || ($what[0]["user"]                             == $this->GetUserName()))    
        ...
        if ($file_size < $maxfilesize*1024)
        {
          ...
          if (!$forbid)
          {
            ...
            $this->Query("insert into ".$this->config["table_prefix"]."upload set ".
                   "page_id = '".quote($is_global?"0":$this->page["id"])."', ".
                   "filename = '".quote($small_name)."', ".
                   "description = '".quote($description)."', ".
                   "filesize = '".quote($file_size)."',".
                   "picture_w = '".quote($size[0])."',".
                   "picture_h = '".quote($size[1])."',".
                   "file_ext = '".quote(substr($ext,0,10))."',".
                   "user = '".quote($user["name"])."',".
                   "uploaded_dt= '".quote(date("Y-m-d H:i:s"))."' ");
