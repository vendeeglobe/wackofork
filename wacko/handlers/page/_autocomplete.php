<?php
  ...
  // 1. перевести адрес в супертаг и анврапнуть
  $q = ltrim($q, "/");
  $supertag1 = $this->NpjTranslit( $this->UnwrapLink($q) );
  $supertag2 = $this->NpjTranslit( $q );

  // 2. ходим в БД два раза
  $limit = 10;

  $pages1 = $this->LoadAll("select ".$this->pages_meta." from ".
            $this->config["table_prefix"]."pages where ".
            " supertag like '".quote($supertag1)."%' and comment_on = '' order by supertag asc limit $limit");
  $pages2 = $this->LoadAll("select ".$this->pages_meta." from ".
            $this->config["table_prefix"]."pages where ".
            " supertag like '".quote($supertag2)."%' and comment_on = '' order by supertag asc limit $limit");

  // 3. обрезаем по правам
  $pages = array();
  $cnt=0;
  if ($pages1)
